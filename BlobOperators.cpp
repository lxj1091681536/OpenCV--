...
/***************************************************************************
  Implementaci?de les classes per al c鄉cul de caracter韘tiques sobre el blob
...
- PAR繫ETRES:
- RESULTAT:
	- retorna la longitud del per韒etre convex del blob. Si el blob no t?coordenades
	  associades retorna el per韒etre normal del blob.
- RESTRICCIONS:
- AUTOR: Ricard Borr鄐
...
double CBlobGetElongation::operator()(CBlob &blob)
{
	...
	// error intr韓sec en els c鄉culs de l'鄏ea i el per韒etre 
	else
...
/**
	Calcula la dist鄋cia entre un punt i el centre del blob
*/
/**
...
- PAR繫ETRES:
	- fabricaOperadorsBlob: f郻rica on es registraran els operadors
- RESULTAT:
	- Modifica l'objecte fabricaOperadorsBlob
- RESTRICCIONS:
	- Nom閟 es registraran els operadors de blob.h. Si se'n volen afegir, cal afegir-los amb 
	  el m鑤ode Register de la f郻rica.
- AUTOR: rborras
