...
- FUNCIONALITAT: Constructor a partir d'una imatge. Inicialitza la seqncia de blobs 
			   amb els blobs resultants de l'an鄉isi de blobs de la imatge.
- PAR繫ETRES:
	- source: imatge d'on s'extreuran els blobs
	- mask: m鄐cara a aplicar. Nom閟 es calcularan els blobs on la m鄐cara sigui 
			diferent de 0. Els blobs que toquin a un pixel 0 de la m鄐cara seran 
...
/**
- FUNCI? CBlobResult
- FUNCIONALITAT: Constructor de c騪ia. Inicialitza la seqncia de blobs 
			   amb els blobs del par鄊etre.
- PAR繫ETRES:
...
CBlobResult::CBlobResult( const CBlobResult &source )
{
	...
	// creem el nou a partir del passat com a par鄊etre
	m_blobs = Blob_vector( source.GetNumBlobs() );
	...
	{
		// no podem cridar a l'operador = ja que Blob_vector 閟 un 
...
- PAR繫ETRES:
- RESULTAT:
	- Allibera la mem騬ia reservada de cadascun dels blobs de la classe
- RESTRICCIONS:
- AUTOR: Ricard Borr鄐
- DATA DE CREACI? 25-05-2005.
- MODIFICACI? Data. Autor. Descripci?
...
{
	// si ja s髇 el mateix, no cal fer res
	if (this != &source)
	{
...
- AUTOR: Ricard Borr鄐
- DATA DE CREACI? 25-05-2005.
- NOTA: per la implementaci? els blobs del par鄊etre es posen en ordre invers
- MODIFICACI? Data. Autor. Descripci?
*/
/**
- FUNCTION: + operator
...
{	
	...
	// reservem mem騬ia per als nous blobs
	resultat.m_blobs.resize( resultat.GetNumBlobs() + source.GetNumBlobs() );

	// declarem els iterador per rec騬rer els blobs d'origen i desti
	Blob_vector::const_iterator pBlobsSrc = source.m_blobs.begin();
...
/**
- FUNCI? GetNumber
- FUNCIONALITAT: Calcula el resultat especificat sobre un 鷑ic blob de la classe
- PAR繫ETRES:
	- evaluador: Qualsevol objecte derivat de COperadorBlob
	- indexblob: n鷐ero de blob del que volem calcular el resultat.
- RESULTAT:
...
/**
- FUNCI? Filter
- FUNCIONALITAT: Filtra els blobs de la classe i deixa el resultat amb nom閟 
...
- PAR繫ETRES:
	...
	- LowLimit:  valor num鑢ic per a la comparaci?(Condition) de la mesura (FilterType)
	- HighLimit: valor num鑢ic per a la comparaci?(Condition) de la mesura (FilterType)
				 (nom閟 t?sentit per a aquelles condicions que tenen dos valors 
							...
							
{
	...
	if( &dst == this ) 
	{
		// esborrem els primers blobs ( que s髇 els originals )
...
/**
- FUNCI? GetNthBlob
- FUNCIONALITAT: Retorna l'en鑣sim blob segons un determinat criteri
- PAR繫ETRES:
...
- RESULTAT:
	- retorna el blob nBlob a dst ordenant els blobs de la classe segons el criteri
	  en ordre DESCENDENT. Per exemple, per obtenir el blob major:
		...
		GetNthBlob( CBlobGetArea(), 1, blobMajor ); (segon blob m閟 gran)
- RESTRICCIONS:
- AUTOR: Ricard Borr鄐
- DATA DE CREACI? 25-05-2005.
- MODIFICACI? Data. Autor. Descripci?
...
- PAR繫ETRES:
- RESULTAT: 
	- Allibera tota la mem騬ia dels blobs
- RESTRICCIONS:
- AUTOR: Ricard Borr鄐 Navarra
- DATA DE CREACI? 25-05-2005.
- MODIFICACI? Data. Autor. Descripci?
*/
/*
...
/**
- FUNCI? RaiseError
- FUNCIONALITAT: Funci?per a notificar errors al l'usuari (en debug) i llen鏰
			   les excepcions
- PAR繫ETRES:
	- errorCode: codi d'error
- RESULTAT: 
	- Ensenya un missatge a l'usuari (en debug) i llen鏰 una excepci?
- RESTRICCIONS:
- AUTOR: Ricard Borr鄐 Navarra
- DATA DE CREACI? 25-05-2005.
- MODIFICACI? Data. Autor. Descripci?
...
/**
- FUNCI? PrintBlobs
- FUNCIONALITAT: Escriu els par鄊etres (鄏ea, per韒etre, exterior, mitjana) 
...
{
	double_stl_vector area, /*perimetre,*/ exterior, compacitat, longitud, latitude, 
					  externPerimeter, perimetreConvex, perimetre, axisratio, mean, stddev, orientation;
	int i;
	FILE *fitxer_sortida;
	...
	axisratio = GetSTLResult(CBlobGetAxisRatio());
	latitude = GetSTLResult(CBlobGetBreadth());
	...
	stddev = GetSTLResult( CBlobGetStdDev());
	orientation = GetSTLResult(CBlobGetOrientation());
	...
	{
		fprintf( fitxer_sortida, "blob %d ->\t area=%7.0f\t perimetre=%8.2f (%8.2f externPerimeter)\t perimetreConvex=%8.2f\t exterior=%.0f\t compacitat=%3.2f\t longitud=%8.2f\t latitude=%8.2f\t axisratio=%5.3f\t mean=%5.3f\t stddev=%5.3f\t orientation=%5.3f\n",
			i, area[i], perimetre[i], externPerimeter[i], perimetreConvex[i], exterior[i], compacitat[i], longitud[i],latitude[i], axisratio[i], mean[i], stddev[i], orientation[i] );
	}
