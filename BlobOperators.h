...
typedef COperadorBlob funcio_calculBlob;
	...
	/**
		Funci?per comparar dos identificadors dins de la f郻rica de COperadorBlobs
	*/
...
//! Classe per calcular l'鄏ea d'un blob
...
//! Classe que diu si un blob 閟 extern o no
...
//! Classe per calcular la desviaci?est鄋dard dels nivells de gris d'un blob
...
//! Classe per calcular la difer鑞cia en X del blob
...
//! Classe per calcular l'鄏ea del poligon convex d'un blob
...
//! Classe per a calcular la x m韓ima
...
//! Classe per a calcular la x m鄕ima
...
//! Classe per a calcular la y m韓ima
...
//! Classe per a calcular la y m鄕ima
...
//! Classe per calcular la dist鄋cia entre el centre del blob i un punt donat
...
private:
	// coordenades del punt on volem calcular la dist鄋cia
	double m_x, m_y;
};
...
//! valors propers a 0 indiquen que la majoria del blob 閟 intern
//! valors propers a 1 indiquen que la majoria del blob 閟 extern
...
//! Classe per calcular el ratio entre l'eix major i menor de la el穕ipse
